...
def plot_cm(cm, classlabels=["<30", "30~60", "60~120", "120~180", "180~240", ">240"], title="Confusion Matrix"):
    plt.figure(figsize=(10, 8))  # Increased figure size for 6x6 matrix
...
def plot_cm_auroc(res_path, model_path="model.pt", test_dataset_path="test_dataset.pt"):
    ...
    sns.set(font_scale=1.2)
    ax = sns.heatmap(cm, annot=cm, fmt='g', cmap="Blues", 
                     xticklabels=["<30", "30~60", "60~120", "120~180", "180~240", ">240"],
                     yticklabels=["<30", "30~60", "60~120", "120~180", "180~240", ">240"])
    plt.xlabel("Predicted Values")
    plt.ylabel("Actual Values")
...
def plot_cm_auroc_subplots(res_path, model_path="model.pt", test_dataset_path="test_dataset.pt", ax=None, fontsize=1.2, annotsize=1.2):
    ...
    sns.heatmap(cm, annot=cm, fmt='g', cmap="Blues", 
                xticklabels=["<30", "30~60", "60~120", "120~180", "180~240", ">240"],
                yticklabels=["<30", "30~60", "60~120", "120~180", "180~240", ">240"],
                ax=ax, cbar=False, annot_kws={'size': annotsize})
...
def combined_calibration_plot(probabilities, probabilities_calibrated, targets, ax=None, title=None, n_bins=10):
    ...
    if n_classes != 6:  # Updated check for 6 classes
        raise ValueError("This function is designed for 6 classes. Adjust the code for a different number of classes.")
    
    # Convert targets to one-hot encoding
    targets_one_hot = torch.zeros(n_samples, n_classes).scatter_(1, targets.unsqueeze(1), 1)
    
    if ax is None:
        _, ax = plt.subplots(2, 3, figsize=(18, 12))  # Updated to 2x3 grid
    
    for class_idx in range(n_classes):
        current_ax = ax[class_idx // 3, class_idx % 3]  # Updated indexing for 2x3 grid
...
def compute_custom_auroc(y_true, y_probs):
    ...
    # For classes 0,1 vs rest
    binary_labels = (y_true <= 1).astype(int)
    auroc_01_vs_rest = roc_auc_score(binary_labels, y_probs[:, 0] + y_probs[:, 1])
    
    # For classes 0,1,2 vs rest
    binary_labels = (y_true <= 2).astype(int)
    auroc_012_vs_rest = roc_auc_score(binary_labels, y_probs[:, 0] + y_probs[:, 1] + y_probs[:, 2])
    
    # For classes 0,1,2,3 vs rest
    binary_labels = (y_true <= 3).astype(int)
    auroc_0123_vs_rest = roc_auc_score(binary_labels, y_probs[:, :4].sum(axis=1))
    
    # For classes 0,1,2,3,4 vs 5
    binary_labels = (y_true <= 4).astype(int)
    auroc_01234_vs_5 = roc_auc_score(binary_labels, y_probs[:, :5].sum(axis=1))
    
    return auroc_0_vs_rest, auroc_01_vs_rest, auroc_012_vs_rest, auroc_0123_vs_rest, auroc_01234_vs_5

def compute_custom_aucpr(y_true, y_probs):
    # For class 0 vs rest
    binary_labels = (y_true == 0).astype(int)
    precision, recall, _ = precision_recall_curve(binary_labels, y_probs[:,0])
    aucpr_0_vs_rest = auc(recall, precision)
    
    # For classes 0,1 vs rest
    binary_labels = (y_true <= 1).astype(int)
    precision, recall, _ = precision_recall_curve(binary_labels, y_probs[:, :2].sum(axis=1))
    aucpr_01_vs_rest = auc(recall, precision)
    
    # For classes 0,1,2 vs rest
    binary_labels = (y_true <= 2).astype(int)
    precision, recall, _ = precision_recall_curve(binary_labels, y_probs[:, :3].sum(axis=1))
    aucpr_012_vs_rest = auc(recall, precision)
    
    # For classes 0,1,2,3 vs rest
    binary_labels = (y_true <= 3).astype(int)
    precision, recall, _ = precision_recall_curve(binary_labels, y_probs[:, :4].sum(axis=1))
    aucpr_0123_vs_rest = auc(recall, precision)
    
    # For classes 0,1,2,3,4 vs 5
    binary_labels = (y_true <= 4).astype(int)
    precision, recall, _ = precision_recall_curve(binary_labels, y_probs[:, :5].sum(axis=1))
    aucpr_01234_vs_5 = auc(recall, precision)
    
    return aucpr_0_vs_rest, aucpr_01_vs_rest, aucpr_012_vs_rest, aucpr_0123_vs_rest, aucpr_01234_vs_5
...
def compute_custom_accuracy_combine_classes(y_true, y_pred):
    ...
    # For classes 0,1 vs rest
    binary_labels = (y_true <= 1).astype(int)
    preds_01_vs_rest = (y_pred <= 1).astype(int)
    acc_01_vs_rest = (binary_labels == preds_01_vs_rest).mean()
    
    # For classes 0,1,2 vs rest
    binary_labels = (y_true <= 2).astype(int)
    preds_012_vs_rest = (y_pred <= 2).astype(int)
    acc_012_vs_rest = (binary_labels == preds_012_vs_rest).mean()
    
    # For classes 0,1,2,3 vs rest
    binary_labels = (y_true <= 3).astype(int)
    preds_0123_vs_rest = (y_pred <= 3).astype(int)
    acc_0123_vs_rest = (binary_labels == preds_0123_vs_rest).mean()
    
    # For classes 0,1,2,3,4 vs 5
    binary_labels = (y_true <= 4).astype(int)
    preds_01234_vs_5 = (y_pred <= 4).astype(int)
    acc_01234_vs_5 = (binary_labels == preds_01234_vs_5).mean()
    
    return acc_0_vs_rest, acc_01_vs_rest, acc_012_vs_rest, acc_0123_vs_rest, acc_01234_vs_5
...
def get_all_metrics(predictions, targets):
    ...
    try:
        auroc_0_vs_rest, auroc_01_vs_rest, auroc_012_vs_rest, auroc_0123_vs_rest, auroc_01234_vs_5 = compute_custom_auroc(targets, predictions)
    except ValueError:
        auroc_0_vs_rest, auroc_01_vs_rest, auroc_012_vs_rest, auroc_0123_vs_rest, auroc_01234_vs_5 = np.nan, np.nan, np.nan, np.nan, np.nan
    
    try:
        aucpr_0_vs_rest, aucpr_01_vs_rest, aucpr_012_vs_rest, aucpr_0123_vs_rest, aucpr_01234_vs_5 = compute_custom_aucpr(targets, predictions)
    except ValueError:
        aucpr_0_vs_rest, aucpr_01_vs_rest, aucpr_012_vs_rest, aucpr_0123_vs_rest, aucpr_01234_vs_5 = np.nan, np.nan, np.nan, np.nan, np.nan
    
    acc_0_vs_rest, acc_01_vs_rest, acc_012_vs_rest, acc_0123_vs_rest, acc_01234_vs_5 = compute_custom_accuracy_combine_classes(targets, pred_labels)
    ...
    res_dict = {
        ...
        "Accuracy_01_rest": acc_01_vs_rest,
        "Accuracy_012_rest": acc_012_vs_rest,
        "Accuracy_0123_rest": acc_0123_vs_rest,
        "Accuracy_01234_5": acc_01234_vs_5,
        "ROC_AUC_0_rest": auroc_0_vs_rest,
        "ROC_AUC_01_rest": auroc_01_vs_rest,
        "ROC_AUC_012_rest": auroc_012_vs_rest,
        "ROC_AUC_0123_rest": auroc_0123_vs_rest,
        "ROC_AUC_01234_5": auroc_01234_vs_5,
        "PR_AUC_0_rest": aucpr_0_vs_rest,
        "PR_AUC_01_rest": aucpr_01_vs_rest,
        "PR_AUC_012_rest": aucpr_012_vs_rest,
        "PR_AUC_0123_rest": aucpr_0123_vs_rest,
        "PR_AUC_01234_5": aucpr_01234_vs_5,
        "ECE": ece.item()
    }
    return res_dict